...
function renderTaskList(container, tasks, emptyMsg) {
  ...
  function renderRedacao() {
    ...
    tabsEl.innerHTML = '';
    // Lê a variável ESSAY_THEMES global do data.js
  ...
  async function callGemini(prompt) {
    ...
    if (!res.ok) throw new Error(data.error?.message || 'Erro da API do Gemini');
  ...
  function renderRecursos() {
    ...
    // Lê a variável STUDY_TOPICS global do data.js
    Object.entries(STUDY_TOPICS).forEach(([materia, topicos]) => {
      ...
      card.innerHTML = `<div class="subject-header">
          <div class="subject-header-left">
            ...
            <div><div class="subject-name">${materia}</div><div class="subject-count">${topicos.length} grandes tópicos</div></div>
          </div>
          <svg class="subject-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg>
        </div>
        <div class="subject-body">${topicos.map(t => `<div class="topic-row"><span class="topic-name">${t}</span><button class="btn btn-xs btn-outline btn-ia-topic" data-materia="${materia}" data-topico="${t}">Aprender com Professor IA</button></div>`).join('')}</div>`;
